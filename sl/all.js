...
function refine(_space, _writer) {
  ...
  let reduction_limit = op.read_universal(
    "reduction_limit",
    SASEUL.Enc.ZERO_ADDRESS,
    reduction_limit_default
  );
  let multiplier = op.read_universal(
    "multiplier",
    SASEUL.Enc.ZERO_ADDRESS,
    "1"
  );
  let season_supply = op.read_universal(
    "season_supply",
    SASEUL.Enc.ZERO_ADDRESS,
    "0"
  );
  let total_supply = op.read_universal(
    "total_supply",
    SASEUL.Enc.ZERO_ADDRESS,
    "0"
  );
  ...
  let next_reduction_limit = op.precise_mul(
    reduction_limit_default,
    next_multiplier,
    0
  );
  ...
  err_msg =
    "The amount to be refined must be greater than or equal to the amount of resources you have. ";
  method.addCondition(op.legacy_condition(condition, err_msg));
  ...
  next_reduction_limit = op.if(
    next_condition,
    next_reduction_limit,
    reduction_limit
  );
  update = op.write_universal(
    "reduction_limit",
