...
function fee(_space, _writer) {
  ...
  let recycle_resource = op.read_local(
    "recycle_resource",
    SASEUL.Enc.ZERO_ADDRESS,
    "0"
  );
  ...
  let err_msg = op.concat([
    "Insufficient balance to pay the fee. (Fee: ",
    fee,
    ")",
  ]);
...
function publish() {
  ...
  method.name("Publish");
  method.version("2");
  method.space(SASEUL.Enc.hash("Hello dan & hans"));
  method.writer(SASEUL.Enc.ZERO_ADDRESS);
  ...
  condition = op.if(
    ...
    op.if(op.eq(type, "request"), op.gt(version, request_version), false)
  );
